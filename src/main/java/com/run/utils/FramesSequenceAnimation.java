...
public class FramesSequenceAnimation {
    ...
    private boolean isEnd;//是否播放完
    private int loopDuration;//每次循环的间隔
    ...
    private int getNext() {
        ...
        if(mIndex==mFrames.length-1)isEnd = true;
    ...
    public synchronized void start() {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                //新开线程去读下一帧
                if(!(isEnd&&!isLoop)) {
                    mHandler.postDelayed(this, isEnd && loopDuration != 0 ? loopDuration : mDelayMillis);
    ...
    public synchronized void stop() {
        mShouldRun = false;
        ImageView imageView = mSoftReferenceImageView.get();
        if(imageView!=null)imageView.setImageResource(mFrames[0]);
    }
     ...
     * @return
     */
    public static int[] getData(int resId,Context context){
